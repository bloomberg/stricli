...
While Stricli currently uses `vitest` for its own tests, any test framework/library can be used. As such, imports for `describe`/`it` and `expect` have been elided in the examples below to make them more readable.
