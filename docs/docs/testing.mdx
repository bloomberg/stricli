...
Stricli commands are designed to run with a [provided context](./structure/isolated-context.mdx), which makes it easier to write isolated tests. Most commands can be tested in one of two ways, depending on construction:
...
If your application makes use of [custom data in the context](./structure/isolated-context.mdx#custom-data), then your equivalent `buildContextForTest` function should include that data as well.

## Running the Application

To run a Stricli application, the [`run`](../packages/core/functions/run) function takes in the application, the inputs as an array of strings, and the context. If a "mock" version of the context can be constructed, then the application can be tested with the [`run`](../packages/core/functions/run) function.

:::tip[A Note For Inputs]

At runtime, the `inputs` array is derived from `process.argv`. As such, it has already handled "tokenizing" the raw inputs and trimming the runtime/file name (if present). This means that in order to test `app cmd foo "bar baz"` the inputs array to [`run`](../packages/core/functions/run) should be `["cmd", "foo", "bar baz"]`.
...
The [`run`](../packages/core/functions/run) function will always return a `Promise<void>`, as it is intended for use on the command line. If the application (or a command) throws an error it will be captured and redirected to stderr.
...
Commands that take full advantage of the [lazy loader pattern](./structure/command-routing/commands.mdx#lazy-loader) (or the [direct function pattern](./structure/command-routing/commands.mdx#direct-func), if the function is exported separately) can be tested directly by calling the implementation function.
...
While [`run`](../packages/core/functions/run) captures thrown exceptions to format them nicely for the command line, implementation functions should not. This means that if an exception is thrown, it will be thrown to the test and must be handled there.
