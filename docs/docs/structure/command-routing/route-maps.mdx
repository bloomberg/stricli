...
The `routes` object is a simple mapping from route names to targets. The route names specified here are affected by the [scanner case style config](../../features/configuration.mdx#scanner-case-style) when provided at runtime.
