...
The Stricli framework was designed to give applications the maximum amount of flexibility within the predefined constraints. When [building an application](../structure/command-routing/index.mdx#applications), you can specify additional configurations to control the application behavior in different ways.
...
-   `exceptionWhileLoadingCommandContext`
    ...
    -   This function is called if an exception is thrown while loading the command context. This will only happen if the context passed to [`run`](../../packages/core/functions/run) has a `forCommand` method to provide a custom command context. This generally implies that there is something wrong with the application itself and how it was built/structured.
