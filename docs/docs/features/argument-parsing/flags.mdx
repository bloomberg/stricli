...
Here's an example with `withNegated: false`, where the negated flag format is disabled:

import BooleanFlagNoNegationCode from "./examples/boolean-flag-no-negation.txt";

<StricliPlayground filename="boolean-flag-no-negation" rootExport="root" appName="run" defaultInput="">
    {BooleanFlagNoNegationCode}
</StricliPlayground>
