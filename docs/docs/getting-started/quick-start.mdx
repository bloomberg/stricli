...
Stricli applications can be executed directly in source, or in a separate script. This template generates a `src/bin/cli.ts` file to invoke [`run`](../../packages/core/functions/run) so that the rest of the source only contains exports without side-effects. Try running your new app by calling the compiled bin script at `dist/cli.js` with `--help`.
