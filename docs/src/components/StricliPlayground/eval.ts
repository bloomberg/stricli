...
type RegisterCallback = (require: (mod: string) => unknown, exports: Exports, ...globals: unknown[]) => void;

export function evaluateFiles(
    files: EmittedFiles,
    entrypoint: string,
    shadowedGlobals: Record<string, unknown>,
    externalImport: (mod: string) => unknown,
): Exports {
    const modules: Record<string, RegisterCallback> = {};
    const exportsByModule: Record<string, Exports> = {};

    const globals = Object.entries(shadowedGlobals);
    ...
    const wrappedFiles = Object.entries(files).map(([name, content]) => {
        const params = ["require", "exports", ...globals.map(([key]) => key)].join(", ");
        return `register(${JSON.stringify(name)}, function (${params}) {\n${content}\n});`;
    });
