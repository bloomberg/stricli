...
import { ConsoleRedirect, parseArgv, PlaygroundApp, proposeCompletions, runApplication } from "./actions";
...
export default function StricliPlayground({
    ...
    const onWorkspaceChange = useCallback(
        (emittedFiles: EmittedFiles) => {
            if (Object.keys(emittedFiles).length > 0) {
                const consoleRedirect = new ConsoleRedirect();
                const globals = {
                    console: consoleRedirect.console,
                };
                const exports = evaluateFiles(emittedFiles, `file:///${modelDirectory}/app.js`, globals, importCore);
                if (exports) {
    ...
    const tooltipContent = (
        <div className="stricli-tooltip">
            This playground allows you to experiment with Stricli applications directly in your browser. It compiles
            TypeScript and runs as-you-type.
            <br />
            <br />
            Navigate to different files using the tabs and use the terminal below to run commands and see output. Arrow
            keys are supported to scroll through input history.
        </div>
    );
    ...
    return (
        <Admonition type="note" title={titleContent} icon={"\u2139\ufe0f"}>
            ...
            <Terminal
                ...
                executeInput={async (input) => {
                    ...
                    return [...lines.toReversed(), [input, "stdin", app.config.name]];
                }}
